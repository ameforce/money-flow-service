...
def _repair_legacy_sqlite_schema() -> None:
    ...
    with engine.begin() as conn:
        ...
        verification_columns = _sqlite_column_names(conn, "email_verification_tokens")
        if verification_columns and "verification_code_hash" not in verification_columns:
            conn.execute(text("ALTER TABLE email_verification_tokens ADD COLUMN verification_code_hash VARCHAR(128)"))
        if verification_columns and "pending_password_hash" not in verification_columns:
            conn.execute(text("ALTER TABLE email_verification_tokens ADD COLUMN pending_password_hash VARCHAR(255)"))
        if verification_columns and "pending_display_name" not in verification_columns:
            conn.execute(text("ALTER TABLE email_verification_tokens ADD COLUMN pending_display_name VARCHAR(120)"))
        if verification_columns and "registration_continuation_hash" not in verification_columns:
            conn.execute(text("ALTER TABLE email_verification_tokens ADD COLUMN registration_continuation_hash VARCHAR(128)"))
