...
def _holding_owner_identity_key(owner_user_id: str | None, owner_name: str | None) -> str:
    normalized_owner_user_id = normalize_optional_text(owner_user_id)
    if normalized_owner_user_id:
        return normalized_owner_user_id
    return f"__legacy_null_owner__:{normalize_optional_text(owner_name)}"
...
def _same_owner_identity(
    ...
    *,
    ...
    return _holding_owner_identity_key(holding.owner_user_id, holding.owner_name) == _holding_owner_identity_key(
        owner_user_id,
        owner_name,
    )
...
def _find_duplicate_holding(
    ...
    *,
    ...
    normalized_market_symbol = _normalize_identity_text(market_symbol).upper()
    normalized_account = _normalize_identity_text(account_name)
    candidates = db.scalars(
        select(Holding).where(
            Holding.household_id == household_id,
            Holding.asset_type == asset_type,
            func.upper(func.trim(Holding.market_symbol)) == normalized_market_symbol,
            func.trim(func.coalesce(Holding.account_name, "")) == normalized_account,
        )
    ).all()
