...
class WorkbookImporter:
    ...
    def _apply_holdings(
        ...
        def resolve_owner(value: str | None) -> tuple[str | None, str]:
            owner_user_id, normalized_owner, status = self._resolve_owner_from_lookup(owner_lookup, value)
            if status == "ambiguous":
                ambiguous_owner_names.add(normalized_owner)
            elif status == "invalid":
                invalid_owner_names.add(normalized_owner)
            return owner_user_id, normalized_owner
    ...
    def _load_household_member_owner_lookup(self, db: Session, household_id: str) -> dict[str, list[str]]:
        rows = db.execute(
    ...
    def _resolve_owner_from_lookup(
        ...
        if len(user_ids) > 1:
            return None, normalized_owner, "ambiguous"
        return None, normalized_owner, "invalid"
