...
| IMPORT-PREFLIGHT | Import file selected and preflight validation | Import upload panels, disabled action helper, progress status, validation report | Users must know when a file is required, when validation is running, and what was checked before applying. | `e2e/specs/import.spec.js` no-file, dry-run, large-report checks |
