...
| MUI-008 | P2 | Collaboration and import mode navigation | Collaboration tab-like controls still omit selected/panel relationships and arrow-key behavior. The import mode selector's invalid `tablist`/ordinary-button combination was corrected to a named exclusive button group with `aria-pressed`, but the combined W2 interaction contract remains open until both surfaces are verified together. | `frontend/src/pages/collaboration/CollaborationInviteTables.jsx:57-67`, `frontend/src/pages/collaboration/CollaborationInviteTables.jsx:141-151`, `frontend/src/pages/importing/WorkbookImportPanel.jsx` | W2 task/accessibility | Open | Pending: `.omo/evidence/mobile-uiux-v0.1.49/MUI-008/` must prove roles, selected state, relationships, and keyboard behavior for collaboration plus the import selector. |
