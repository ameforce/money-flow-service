...
`npm run e2e`, `npm run e2e:raw`, `npm run e2e:headed`, `scripts/run_e2e_with_orchestrator.py`
가 해당 런타임 라이브러리를 자동으로 `LD_LIBRARY_PATH`에 prepend 합니다.
추가 수동 export 없이 재실행하면 됩니다.
...
- 배포(dev/prod/production)에서는 `EMAIL_DELIVERY_MODE`를 사용하지 않으며, SMTP 인증 메일 전송 실패 시 회원가입이 차단됩니다.
- debug-token 모드 E2E 통과는 앱 플로우 검증 결과이며, 외부 수신 성공은 upstream relay/provider 수신함 또는 `mail:local:logs`의 `status=sent/deferred/bounced`로 별도 확인해야 합니다.
...
- SMTP 필수 환경변수, SES/Google SPF·DKIM·DMARC 체크리스트, Mailpit 내부 캡처와 외부 도착 증거 구분은 `docs/mail-delivery-troubleshooting-and-setup.md`를 참고하세요.
