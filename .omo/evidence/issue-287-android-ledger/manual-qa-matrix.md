...
| Month switch | Edit the transaction month while already on the 거래 tab, then press Enter into a long target month | The month does not reload before Enter, and after applying it re-anchors to the latest row instead of leaving the oldest rows at the top | `red-month-apply-anchor-e2e.txt`, `green-month-apply-anchor-e2e.txt`, `transactions-spec-e2e.txt` |
